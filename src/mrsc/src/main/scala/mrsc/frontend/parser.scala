...
// This file was copypasted from graphsc
...
sealed trait Expr {
  ...
  def toTerm(map: Map[String, Int] = Map()): Term = this match {
    case ExprLambda(v :: vs, body) => 
      Abs(ExprLambda(vs, body).toTerm(map.mapValues(_ + 1) + (v -> 0)))
    case ExprLambda(Nil, body) => body.toTerm(map)
    case ExprFun(name) => GVar(name)
    case ExprVar(name) => map.get(name).fold[Term](GVar(name))(BVar(_))
    case ExprCall(ExprConstr(name), args) => Ctr(name, args.map(_.toTerm(map)))
    case ExprConstr(name) => Ctr(name, Nil)
    case ExprUnused() => Ctr("_|_", Nil)
    case ExprCall(fun, args) => (fun.toTerm(map) /: args.map(_.toTerm(map)))(App(_, _))
    case ExprCaseOf(expr, cases) => 
...
case class Program(
  ...
  def toGContext: GContext = {
    if(!defs.values.forall(_.size == 1))
      throw new Exception("Multiple definitions are not supported")
    defs.mapValues(_(0).toTerm())
  }
}
